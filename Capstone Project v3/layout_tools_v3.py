...
def draw_stationing_leaders_for_points(map_frame, point_event_features):
    """
    Creates one temporary in_memory point feature class containing
    all visible point events in the current map-series page.
    """

    try:
        extent = map_frame.camera.getExtent()
        sr = map_frame.map.spatialReference

        out_fc = r"in_memory\visible_page_events"
        ...
        arcpy.management.AddField(out_fc, "Chainage", "TEXT", field_length=20)

        inserted = 0

        with arcpy.da.InsertCursor(out_fc, ["SHAPE@", "Chainage"]) as icur:
                ...
                with arcpy.da.SearchCursor(point_fc, ["SHAPE@", "Chainage"]) as scur:
                    for shape, chainage in scur:
                        ...
                        if (
                            ...
                            icur.insertRow([pt, chainage])
                            inserted += 1

        arcpy.AddMessage(f"Visible events found on page: {inserted}")

        if inserted > 0:
            map_frame.map.addDataFromPath(out_fc)

    except Exception as e:
        arcpy.AddWarning(f"Could not process visible point events: {e}")