...
def draw_point_ticks_and_labels(
    ...
    label_top_y,  # y where labels sit inside the top band
    ...
    for i, rec in enumerate(point_records, start=1):
        ...
        if i % 2 == 1:
            ...
            current_label_y = label_top_y
            # Label grows upward from label_top_y (sits inside the top band)
            anchor = "BottomCenterPoint"
        else:
            # Even record — tick at the top edge of the bottom band
            tick_band_y = band_y_bottom
            current_label_y = label_bottom_y
            # Label grows downward from label_bottom_y (sits inside bottom band)
            anchor = "TopCenterPoint"
