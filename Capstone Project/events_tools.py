...
def create_intersections_and_overlaps(
    ...
    point_intersections=[] #it will store features created as points or which intersected the route
    line_overlaps=[] #will store features that overlap the route

    route_name=os.path.splitext(os.path.basename(route_fc))[0] #extracting the route name
    ...
    for layer in analysis_layers:
        try: #try is used for error handling so that if one layer which was provided fails, the whole tool does not crash
            ...
            if layer_name.lower()==route_name.lower(): #Here, the code is checking if layer being checked has same name with route and skips
            ...
            if shape_type in ["Polyline", "Polygon"]:
                ...
                arcpy.analysis.Intersect([route_fc, layer], overlap_out, output_type="LINE")
            ...
            arcpy.AddWarning(f"Skipped layer {layer}: {e}") #if something went wrong for specific layer, ArcGIS shows warning message
...
def add_chainage_to_event_tables(
        point_event_tables,
        line_event_tables
):
    code_block=chainage_code_block()#calls function above and stores the returned python code in code_block
    ...
    for table in point_event_tables:
        ...
        arcpy.management.CalculateField(
            table, "Chainage", "chain(!MEAS!)", "PYTHON3", code_block #This calculates Chainage from MEAS
        )
