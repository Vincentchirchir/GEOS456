...
def remove_duplicate_station_measure(
    ...
    with arcpy.da.SearchCursor(station_table, ["StationID", measure_field]) as Scursor:
        for station_id, meas in Scursor:
            key=round(float(meas), 6) if meas is not None else None #converts the measure value to float and 6 decimal places
    ...
    duplicate_station_ids_set=set(duplicate_station_ids) #This Converts the list to a set for faster lookup during deletion.
...
def join_station_chainage_to_points(
    ...
    point_fields=[f.name for f in arcpy.ListFields(station_points)] #Here the code is getting all fields in the station point feature
    ...
    if fields_to_delete:
        arcpy.management.Delete(station_points, fields_to_delete)
...
#It reuses the function from route.py
    ...
    table_field=[f.name for f in arcpy.ListFields(station_table)]
    if "Chainage" not in table_field:
