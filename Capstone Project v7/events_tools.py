...
def create_intersections_and_overlaps(
    ...
    for idx, layer in enumerate(analysis_layers):
...
def locate_intersections_and_overlaps(
    ...
    return SimpleNamespace(
        pts=point_event_tables,
        lines=line_event_tables,
    )
...
def make_event_layers_from_tables(
    ...
    return SimpleNamespace(
        pts=point_event_features,
        lines=line_event_features,
    )
