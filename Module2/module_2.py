...
#looping and feature class listing
import arcpy
import os

arcpy.env.workspace = r"C:\GIS SCRIPTS\GEOS456\Data.gdb"
arcpy.env.overwriteOutput = True  # so repeated runs don't fail on existing outputs

# 1) List features (polygons whose names start with 'C')
fclist = arcpy.ListFeatureClasses("C*", "Polygon")

# Early exit if nothing matched
if not fclist:
    arcpy.AddWarning("No feature classes matched 'C*' and Polygon in the workspace.")
else:
    # 2) Collect full paths (outside tools)
    data = []
    for fc in fclist:
        data_path = os.path.join(arcpy.env.workspace, fc)
        data.append(data_path)
        print("Found:", data_path)

    # 3) Process each item OUTSIDE the listing loop
    for in_data in data:
        # Create an output name and full output path inside the same GDB
        out_name = os.path.basename(in_data) + "_Buffer"
        out_path = os.path.join(arcpy.env.workspace, out_name)

        # Run Buffer
        arcpy.analysis.Buffer(in_data, out_path, "50 Meters")

        print(f"Buffered: {in_data}  ->  {out_path}")
...
arcpy.management.CreateFeatureclass(workspace, outname, "POLYGON", "", "", "", projection)
