...
class Toolbox(object):
    def __init__(self):
        self.label = "Alignment Sheets"
        self.alias = "Alignment Sheets"
        self.tools = [GenerateAlignmentSheets]


class GenerateAlignmentSheets(object):
    def __init__(self):
    ...
    def getParameterInfo(self):
        ...
        map_series_overlap.value = 10

        export_pdf = arcpy.Parameter(
            displayName="Export Alignment Sheets in PDF",
            name="export_pdf",
            datatype="GPBoolean",
            parameterType="Optional",
            direction="Input",
        ...
        start_measure.description = (
            "Required route start measure. Example: 2100 means station 2+100."
        )
        end_measure.description = (
            "Required route end measure. Leave blank to process to the end."
        )
        tolerance.description = "Tolerance for locating features along the route."
        analysis_layers.description = (
            "Required layers to analyze for intersections and overlaps."
        )
        create_layout.description = "Required. Check to create an alignment layout."
        ...
        create_map_series.description = (
            "Required. Check to create strip-map based map series."
        )
    ...
    def execute(self, parameters, messages):
        ...
        # mini map, legend, and station labels in sync on the first run.
        ...
        if create_layout and layout_result:
            ...
            point_event_features = (
                event_feature_outputs.get("point_event_features", [])
                if event_feature_outputs
                else []
            )
            line_event_features = (
                event_feature_outputs.get("line_event_features", [])
                if event_feature_outputs
                else []
            )

            # Draw the initial layout whenever either event type exists.
            # Some projects contain only overlaps, so tying the first draw to
            # point_event_features alone would make the layout look empty even
            # though valid line overlap content exists.
            if point_event_features or line_event_features:
            ...
            # When a map series is present, the page loop below will rebuild the
            # intersection table and summary per page. Seeding the layout with the
            # full-route records here can make the first exported page look global
            # if ArcGIS retains shared layout text between page redraws, so we
            # clear those sections first and let the page-aware updater own them.
            initial_band_records = (
                []
            ...
            auto_populate_layout(
                ...
                route_start=route_start,
                route_end=route_end,
                band_records=initial_band_records,
            )
            ...
            if create_map_series and layout_result.get("map_series_info"):
                ...
                update_map_series_pages(
                    ...
                    line_event_features=(
                        event_feature_outputs["line_event_features"]
                        if event_feature_outputs
                        else []
                    ),
                    band_geom=band_geom,
